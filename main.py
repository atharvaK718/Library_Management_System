...
connector.execute(
    'CREATE TABLE IF NOT EXISTS Library (BK_NAME TEXT, BK_ID TEXT PRIMARY KEY NOT NULL, AUTHOR_NAME TEXT, '
    ...
    if surety:
        try:
            ...
            mb.showerror('Book ID already in use!',
                         'The Book ID you are trying to enter is already in the database, please alter that book\'s '
                         'record or check any discrepancies on your side')
...
rtf_bg = 'Light Green'  # Right Top Frame Background Color
rbf_bg = '#FFA500'  # Right Bottom Frame Background Color
btn_hlb_bg = 'SkyBlue'  # Background color for Head Labels and Buttons

lbl_font = ('Arial', 14)  # Font for all labels
entry_font = ('Helvetica', 12)  # Font for all Entry widgets
btn_font = ('Verdana', 12)
...
Label(root, text='LIBRARY MANAGEMENT SYSTEM', font=("Noto Sans CJK TC", 15, 'bold'), bg=btn_hlb_bg, fg='Black').pack(
    side=TOP, fill=X)
...
# Right Top Frame
Button(RT_frame, text='Delete book record', font=btn_font, bg=btn_hlb_bg, width=18, command=remove_record).place(x=8,
                                                                                                                 y=30)
Button(RT_frame, text='Delete full inventory', font=btn_font, bg=btn_hlb_bg, width=18, command=delete_inventory).place(
    x=197, y=30)
Button(RT_frame, text='Update book details', font=btn_font, bg=btn_hlb_bg, width=18,
       command=update_record).place(x=386, y=30)
Button(RT_frame, text='Change Book Availability', font=btn_font, bg=btn_hlb_bg, width=20,
       command=change_availability).place(x=575, y=30)

# Right Bottom Frame
Label(RB_frame, text='BOOK INVENTORY', bg=rbf_bg, font=("Noto Sans CJK TC", 15, 'bold')).pack(side=TOP, fill=X)
name_label = Label(root, text='Atharva Khadse', font=("Arial", 10), bg='Grey', fg="Black")
name_label.pack(side=BOTTOM, anchor=W, padx=10, pady=10)
